...
def test_open_password_recovery_page(driver):
...
def test_restore_password_by_email(driver):
    main = MainPage(driver)
    main.open_main()
    main.go_to_account()

    login = LoginPage(driver)
    login.is_opened()  # важно: дождались страницы логина
    login.go_to_forgot_password()
...
def test_password_field_becomes_active_on_visibility_toggle(driver):
    main = MainPage(driver)
    main.open_main()
    main.go_to_account()
